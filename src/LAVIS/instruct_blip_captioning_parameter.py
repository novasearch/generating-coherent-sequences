...
CAPTION_RESULT_FILE = f'captions_{caption_type}_temperature.json'

print(f"Generating captions with «{caption_type}»")

if caption_type not in PROMPT_TYPES:
    exit(1)
...
for recipe_id, _recipe in tqdm(recipe_items, desc='recipes'):
    try:
        ...
        try:
            ...
            for i, step in enumerate(steps):
                ...
                results[caption_type] = model.generate({"image": image, "prompt": PROMPTS[caption_type]}, 
                                                       max_length=77, 
